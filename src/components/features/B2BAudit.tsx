...
import { MOCK_B2B_ALERTS } from '@/mock/data';
        ...
        >
            ...
            <motion.div variants={itemVariants as any} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <Card className="bg-card border-yellow-500/20">
                    ...
                    <CardContent className="text-center">
                        <div className="text-4xl font-display font-bold text-yellow-500">{alerts.length}</div>
                    </CardContent>
                </Card>
            ...
            <motion.div variants={itemVariants as any} className="space-y-4">
                ...
                {alerts.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        {alerts.map((alert) => (
                            <motion.div key={alert.id} layout initial={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.9 }}>
                                <Card className={`border-2 ${alert.status === 'fired' ? 'border-red-500/30 bg-red-500/5' : 'border-yellow-500/30 bg-yellow-500/5'}`}>
                                    ...
                                    <CardContent>
                                        <div className="text-sm mb-4">
                                            Инструмент: <strong className="text-foreground">{alert.tool}</strong>
                                        </div>
